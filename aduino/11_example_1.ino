...
#define _DIST_ALPHA 0.35 //ema weight of new sample (range: 0to1). Setting this value to 1 effectively disables ema filter
...
Servo myservo;
float duty_angle, dist_ema, alpha; 
...
// adjust servo position according to the USS read value
  ...
  if(dist_raw <= 180.0) {
     ...
     duty_angle = _DUTY_MIN + ((dist_raw - 180) * (_DUTY_MAX - _DUTY_MIN)/180);
     myservo.writeMicroseconds(duty_angle);
